# Provide data visualisation and then apply two of the best models from above steps to the following dataset. You can report the results with the most appropriate metricsimport pandas as pd
import numpy as np
...
def create_graph_data(X, y, K=5):
    ...
    # 将标签y转换为从0开始的值
    y_adjusted = y - y.min()  # 将最小值设为0，以避免负值
    y_tensor = torch.tensor(y_adjusted.values, dtype=torch.long)  # 用long类型适配分类任务
